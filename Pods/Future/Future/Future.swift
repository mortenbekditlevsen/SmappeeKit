...
extension Future {
    ...
    public func voe<Q>(transform: T->Optional<Q>, error: Error) -> Future<Q, Error> {
        return Future<Q,Error> { completion in
            self.onComplete { r in
                switch r {
                case .Success(let box):
                    if let value = transform(box.value) {
                        completion(Result(value: value))
                    } else {
                        completion(Result(error: error))
                    }
                case .Failure(let box):
                    completion(Result(error: box.value))
                }
            }
        }
    }
